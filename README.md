...
- The thread gets attached to a local checkout of the same project. spools tries, in order: `--project`, the original path, the original path under your home dir, and then any repo in `~/personal`, `~/code`, `~/src` or `~/projects` with the same git remote. Set `SPOOLS_PROJECT_DIRS` (`:`-separated) to search other dirs instead.
